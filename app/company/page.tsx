  ...
  return (<>
    ...
    <Section title="会社概要" bg="soft">
      <div className="grid md:grid-cols-3 gap-8">
        {["社名：Monitly.AI株式会社","拠点：東京・ベトナム（後日更新）","連絡先：hioki@aradia.asia"].map((t,i)=>(
          <div key={i} className="rounded-2xl border border-neutral-200 bg-white p-6 shadow-sm"><div className="text-sm text-neutral-600">{t}</div></div>
        ))}
      </div>
    </Section>
  </>); }
