    ...
    <header className="bg-gradient-to-b from-[#EAF2FF] to-white">
      <div className="mx-auto max-w-container px-6 md:px-8 lg:px-10 py-4 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-6 text-sm text-neutral-700">
          ...
          <Link href="/contact" className="inline-flex items-center rounded-2xl bg-[#0056FF] text-white px-5 py-2 font-semibold">デモを見る</Link>
        </nav>

        <button className="md:hidden text-neutral-700" onClick={() => setMenuOpen(!menuOpen)} aria-label="Toggle menu">☰</button>
      </div>
      ...
      {menuOpen && (
        <div className="md:hidden bg-white border-t border-neutral-200 px-6 py-4 space-y-3">
          ...
          <Link href="/contact" className="block bg-[#0056FF] text-white px-4 py-2 rounded-xl text-center font-semibold">デモを見る</Link>
        </div>
      )}
    </header>
  );
